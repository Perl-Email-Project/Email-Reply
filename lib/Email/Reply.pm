...
sub _new {
    ...
    $self->{attrib}   = $args{attrib} || (
                          ($self->{to}->name || join($self->{to}->address, '<', '>')) .
                            ' wrote:'
                        );
...
sub _make_headers {
    ...
    $self->{to}->name(
      (Email::Address->parse($self->{original}->header('From')))[0]->name
    ) unless $self->{to}->name;
...
sub _mime {
    ...
    Email::MIME->create(
        header => $self->{header},
        parts  => [
                   Email::MIME->create(body => $self->{reply_body}),
                   $self->{message},
                  ],
    );
}
